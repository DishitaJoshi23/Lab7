describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item >>> button');
    
    for (let i = 0; i < prodItems.length; i++) {
      const innerText = await (prodItems[i].getProperty('innerText'));
      expect(await innerText.jsonValue()).toBe('Remove from Cart');
    }
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
     ...
     */
     const prodItems = await page.$$('product-item >>> button');
      ...
      expect(await cartCountText.jsonValue()).toBe('0');
  }, 20000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item >>> button');
    for (const item of prodItems){
      const innerText = await(item.getProperty('innerText'));
